...
export const authApi = {
  superAdminLogin: (data: { id: string; password: string; role: string }) =>
    apiClient.post(ENDPOINTS.auth.superAdminLogin, data),
  adminLogin: (data: { id: string; password: string; role: string }) =>
    apiClient.post(ENDPOINTS.auth.adminLogin, data),
  departmentManagerLogin: (data: { email: string; password: string; role: string }) =>
    apiClient.post(ENDPOINTS.auth.departmentManagerLogin, data),
  studentLogin: (data: { name: string; subject: string; role: string }) =>
